...
const { addons: w } = __STORYBOOK_MODULE_PREVIEW_API__,
	{ ImplicitActionsDuringRendering: D } =
	...
	const n = { ...U, ...t },
		r = (...c) => {
			if (t.implicit) {
				...
				if (g) {
					const d = !window?.FEATURES?.disallowImplicitActionsInRenderV8,
						O = new D({ phase: g.phase, name: e, deprecated: d });
					if (d) console.warn(O);
					else throw O;
				}
			}
			const o = w.getChannel(),
				s = C(),
				a = 5,
				u = c.map(V),
				E = c.length > 1 ? u : u[0],
...
var h = (e, t) => typeof t[e] > "u" && !(e in t),
	K = (e) => {
		...
		const o = new RegExp(c.argTypesRegex);
		return Object.entries(n)
			.filter(([s]) => !!o.test(s))
			.reduce(
				(s, [a, u]) => (h(a, t) && (s[a] = _(a, { implicit: !0, id: r })), s),
				{},
			);
	},
	L = (e) => {
		...
		return r?.disable || !n
			...
			: Object.entries(n)
					...
					.reduce(
						(c, [o, s]) => (
							h(o, t) && (c[o] = _(typeof s.action == "string" ? s.action : o)),
							c
						),
						{},
					);
