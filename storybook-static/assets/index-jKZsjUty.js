import { c as bn } from "./index-pcvojpNF.js";
var ge = { exports: {} };
((n, a) => {
	((o, i) => {
		i(a);
		...
		function W(e) {
			var t, r;
			return (r = (t = K.exec(e)) === null || t === void 0 ? void 0 : t[0]) !==
				null && r !== void 0
				? r
				: null;
		}
		...
		class U {
			...
			parseInfixIntermediateType(t, r) {
				...
				while (s !== null) (t = s), (s = this.tryParslets(t, r));
				return t;
			}
			tryParslets(t, r) {
		...
		const Gt = g({
			...
			ce = [le, H, z, Mt, Bt, Ct, Yt({ allowTrailingComma: !0 }), Gt, Wt, H];
		function Z({
			...
			return (c, y, T) => {
				...
				const $e = r !== null ? new U(r, c.lexer, c) : c,
					O = $e.parseIntermediateType(m.NAME_PATH);
				...
				return { type: "JsdocTypeNamePath", left: w(T), right: M, pathType: x };
			};
		}
			...
			return g({
				...
				parsePrefix: (r) => {
					...
					if (r.consume("StringValue"))
						...
						let S = "";
						const P = ["Identifier", "@", "/"];
						while (P.some((x) => r.consume(x)))
							(S += y.text), (y = r.lexer.current);
						c = {
							type: "JsdocTypeSpecialNamePath",
							value: S,
							specialType: s,
					...
					const T = new U(e, r.lexer, r),
						J = T.parseInfixIntermediateType(c, m.ALL);
		...
		function ue({
			...
			return g({
				...
				parsePrefix: (c) => {
					...
					const S = c.parseIntermediateType(m.FUNCTION);
					if (e === void 0) J.parameters = Xt(S);
					else {
						if (y && S.type === "JsdocTypeFunction" && S.arrow)
							return (J = S), (J.constructor = !0), J;
						J.parameters = pe(S);
						for (const P of J.parameters)
							if (P.type === "JsdocTypeKeyValue" && !e.includes(P.key))
								throw new Error(
									`only allowed named parameters are ${e.join(", ")} but got ${P.type}`,
								);
					}
					if (c.consume(":")) J.returnType = c.parseType(m.PREFIX);
					...
					if (!r.consume("}")) {
						...
						for (;;) {
							...
							if (
								...
								let P;
								T.type === "JsdocTypeStringValue" && (P = T.meta.quote),
									s.elements.push({
		...
		function me({
			...
			return g({
				...
				parseInfix: (c, y) => {
					...
					let J = !1,
						S = !1;
					s && y.type === "JsdocTypeNullable" && ((J = !0), (y = y.element)),
						r &&
							y.type === "JsdocTypeReadonlyProperty" &&
							((S = !0), (y = y.element));
					const P = (T = c.baseParser) !== null && T !== void 0 ? T : c;
					if (
						...
						P.consume(":");
						let x;
						y.type === "JsdocTypeStringValue" && (x = y.meta.quote);
						const ne = P.parseType(m.KEY_VALUE);
						return (
						...
						if (!t) throw new d(y);
						P.consume(":");
						const x = P.parseType(m.KEY_VALUE);
						return (
							c.acceptLexerState(P),
							{ type: "JsdocTypeJsdocObjectField", left: w(y), right: x }
						);
					}
				},
			});
		...
		const je = [
			...
			Le = g({
				...
				parsePrefix: (e) => (
					e.consume("typeof"),
					{ type: "JsdocTypeTypeof", element: w(e.parseType(m.KEY_OF_TYPE_OF)) }
				),
			}),
			Ht = [
				I({
		...
		const tn = g({
				...
				parsePrefix: (e) => (
					e.consume("keyof"),
					{ type: "JsdocTypeKeyof", element: w(e.parseType(m.KEY_OF_TYPE_OF)) }
				),
			}),
			nn = g({
				name: "importParslet",
		...
		function Ke() {
			return {
				...
				JsdocTypeSpecialNamePath: (e) =>
					`${e.specialType}:${A(e.value, e.meta.quote)}`,
				...
				JsdocTypeObject: (e, t) =>
					`{${e.elements.map(t).join((e.meta.separator === "comma" ? "," : ";") + " ")}}`,
		...
		const fn = {
			...
			JsdocTypeSpecialNamePath: (e) =>
				R(e.specialType + ":" + A(e.value, e.meta.quote)),
			JsdocTypeName: (e) =>
				e.value !== "function"
					? R(e.value)
			...
			JsdocTypeMappedType: v,
			JsdocTypeIndexSignature: v,
			JsdocTypeImport: v,
			JsdocTypeKeyof: v,
			JsdocTypeTuple: v,
			JsdocTypeTypeof: v,
			JsdocTypeIntersection: v,
			JsdocTypeProperty: v,
			JsdocTypePredicate: v,
			JsdocTypeAsserts: v,
		};
		function Tn(e) {
			return q(fn, e);
		...
		const gn = {
			...
			JsdocTypeImport: (e) => ({
				...
				path: {
					type: "STRING_VALUE",
					quoteStyle: j(e.element.meta.quote),
		...
		function he(e, t, r, s, c) {
			...
			for (const T of y) {
				const J = e[T];
				if (J !== void 0)
					if (Array.isArray(J)) for (const S of J) he(S, e, T, s, c);
					else he(J, e, T, s, c);
			}
			c?.(e, t, r);
		}
	...
	C = l((n) => {
		...
		switch ((typeof o < "u" && (i.raw = o), n.name)) {
			...
			case "signature":
				return { ...i, ...kn(n) };
			case "union":
				return n.elements?.every(vn)
					? { ...i, name: "enum", value: n.elements?.map(Sn) }
					: { ...i, name: a, value: n.elements?.map(C) };
			case "intersection":
				return { ...i, name: a, value: n.elements?.map(C) };
			default:
...
l(Me, "mapValues");
var Be = /^['"]|['"]$/g,
	xn = l((n) => n.replace(Be, ""), "trimQuotes"),
	On = l((n) => Be.test(n), "includesQuotes"),
	Ce = l((n) => {
		const a = xn(n);
	...
	In = /^\(.*\) => /,
	B = l((n) => {
		const { name: a, raw: o, computed: i, value: p } = n,
			u = {};
		switch ((typeof o < "u" && (u.raw = o), a)) {
			case "enum": {
				const f = i ? p : p.map((h) => Ce(h.value));
			...
			case "arrayOf":
			case "array":
				return { ...u, name: "array", value: p && B(p) };
			case "object":
				return { ...u, name: a };
			case "objectOf":
				return { ...u, name: a, value: B(p) };
			case "shape":
			case "exact":
				const d = Me(p, (f) => B(f));
				return { ...u, name: "object", value: d };
			case "union":
				return { ...u, name: "union", value: p.map((f) => B(f)) };
			case "instanceOf":
			case "element":
			case "elementType":
			default: {
...
var Rn = l((n) => {
				...
				return (
					n.elements?.every((u) => u.name === "literal")
						? (p = {
								...i,
								name: "enum",
								value: n.elements?.map((u) => Ce(u.value)),
							})
						: (p = { ...i, name: a, value: n.elements?.map(Y) }),
					p
				);
			case "intersection":
	...
	we = l((n) => {
		const { type: a, tsType: o, flowType: i } = n;
		try {
...
function et({ fence: n = "```" } = {}) {
	...
	return l((i) => {
		let p = [[]],
			u = !1;
		for (const d of i)
			Ln.test(d.tokens.description) && !u
				? p.push([d])
				: p[p.length - 1].push(d),
	...
	return l((p) => {
		...
		if (
			...
			o === null &&
				...
				([d.postDelimiter, u] = L(u))),
			o === null)
		)
			return i++, null;
		const f = u.trimRight().endsWith(a.end);
		if (
			(d.delimiter === "" &&
...
function ot(n = "compact") {
	...
	return (o) => {
		let i = 0,
			p = [];
		for (const [f, { tokens: h }] of o.source.entries()) {
			let k = "";
		...
		for (const [f, [h, k]] of p.entries())
			(h.type = k),
				f > 0 &&
					((h.type = h.postDelimiter.slice(d) + k),
					(h.postDelimiter = h.postDelimiter.slice(0, d))),
				([h.postType, h.description] = L(h.description.slice(k.length))),
				u.push(h.type);
		return (
			(u[0] = u[0].slice(1)),
...
function it() {
	const n = l((a, { tokens: o }, i) => (o.type === "" ? a : i), "typeEnd");
	return (a) => {
		const { tokens: o } = a.source[a.source.reduce(n, 0)],
...
function ut({
	...
	const u = nt({ startLine: n, markers: i }),
		d = et({ fence: a }),
		f = rt({ tokenizers: p }),
		h = be(o);
	return (k) => {
		const N = [];
		for (const ae of He(k)) {
			const K = u(ae);
			if (K === null) continue;
			const W = d(K),
				X = W.slice(1).map(f);
			N.push({
				...
				tags: X,
				source: K,
				problems: X.reduce((E, oe) => E.concat(oe.problems), []),
			});
		}
		return N;
	};
...
function Tt(n, a) {
	const o = { params: null, deprecated: null, returns: null, ignore: !1 };
...
function wt(n) {
	...
	return a
		? {
				type: a,
				description: Ee(n.name, n.description),
				getTypeName: l(() => Se(a), "getTypeName"),
			}
		: null;
}
l(wt, "extractReturns");
var F = Je.stringifyRules(),
	Bn = F.JsdocTypeObject;
F.JsdocTypeAny = () => "any";
F.JsdocTypeObject = (n, a) => `(${Bn(n, a)})`;
...
function bt(n, a) {
	if (n != null) {
		const { value: o } = n;
		if (!re(o)) return Nt(o) ? b(a?.name, o) : b(o);
	}
	return null;
}
...
l(xt, "createType");
var Cn = l((n, a) => {
	const { flowType: o, description: i, required: p, defaultValue: u } = a;
	return {
		name: n,
		type: xt(o),
		required: p,
...
function It({ tsType: n, required: a }) {
	...
	return (
		...
		b(["Array", "Record", "signature"].includes(n.name) ? n.raw : o)
	);
}
l(It, "createType");
var Yn = l((n, a) => {
	const { description: o, required: i } = a;
	return {
		name: n,
		type: It(a),
		required: i,
...
function Oe(n, a, o) {
	const { description: i, required: p, defaultValue: u } = o;
	return {
		name: n,
		type: At(a),
		required: p,
...
function G(n, a) {
	if (a?.includesJsDoc) {
		const { description: o, extractedTags: i } = a;
		o != null && (n.description = a.description);
		const p = {
			...i,
